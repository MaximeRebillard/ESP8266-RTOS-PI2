...
dynamic initialization of the array containing the possibilities for S0 S1 S2
calloc() zero-initializes the buffer, while malloc() leaves the memory uninitialized.

*/

Code_array =(int**)malloc(sizeof(int*));

for(int i = 0; i < flexNumber; i++){

  switch(i){
    case 0:
      Code_array[i] = &Code_y0;
      break;
    case 1:
      Code_array[i] = &Code_y1;
      break;
     case 2:
      Code_array[i] = &Code_y2;
      break;
    case 3:
      Code_array[i] = &Code_y3;
      break;
    case 4:
      Code_array[i] = &Code_y4;
      break;

  }
    Code_array[i] = (int*)malloc(sizeof(int));
}
